...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(0, m):
			data = X[i, :]
			diff = data - clusters
			label_temp = np.argsort(np.linalg.norm(diff, axis=1)).item(0)
			label[i] = label_temp
		for cluster_num in range (0,k):
			temp = np.where(label == cluster_num)[0]
			if len(temp) > 0:
				clusters[cluster_num, :] = X[temp].mean(axis =0)
		"*** END YOUR CODE HERE ***"
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	cost_vs_cluster_num, = plt.plot(range(1,21), cost_k_list, "g")
