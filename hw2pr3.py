...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
    ...
    reg_list.sort()
    W_list = [linreg(X_train, y_trian, reg = lb) for lb in reg_list]
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
        train_rmse = np.sqrt(np.linalg.norm((X_train @ W).reshape((-1,1)) + b -y_trian ) ** 2 / m_train)
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
    X_train = np.hstack((np.one_like(y_train), X_train))
    y_val = np.hstack((np.one_like(y_val), X_val))
